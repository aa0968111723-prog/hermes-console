...
export default function LoginScreen({
  ...
  const [mode, setMode] = useState<
    "login" | "register" | "magic" | "forgot" | "reset"
  >(resetToken ? "reset" : "login");
  const resetting = mode === "reset" && !!resetToken;
  ...
  async function submit(action: string, extra: Record<string, string> = {}) {
    ...
    try {
      const payload =
        action === "reset"
          ? { action, token: resetToken, password: extra.password }
          : { action, email: address, ...extra };
    ...
    <main className="auth-gate">
      <section className="auth-card">
        <img
          ...
          height={120}
        />
        <h1>{resetting ? "重設密碼" : "登入 Hermes"}</h1>
        {message && (
        ...
        <form
          onSubmit={(event) => {
            ...
            if (mode === "reset") void submit("reset", { password });
            else if (mode === "login") void submit("login", { password });
