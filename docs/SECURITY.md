...
Login ≠ authorization. APIs check `hermes_session` and workspace membership (`owner` / `admin` / `member`). Changing Hermes, MCP, Canva authorization, runtime bindings, or Zeabur credentials requires owner or admin on the server. Members may use the workspace and read MCP **status**. MCP endpoints, credential references, and tool schemas are operator-only. Hiding a button is not access control.
