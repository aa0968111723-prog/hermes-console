...
本輪驗證（此環境，非 Zeabur 實機）：`npm test` 413 pass / 2 skip；`lint`／`typecheck`／`build`／`check:secrets`／`rehearse`／`test:entry`／`test:ui`／`test:chat`／`test:workbench`／`test:runtime`／`test:gateway` 全過。
