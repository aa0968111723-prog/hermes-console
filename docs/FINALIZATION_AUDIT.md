...
本輪驗證（此環境，非 Zeabur 實機）：`npm test` 415 pass / 2 skip；`lint`／`typecheck`／`build`／`check:secrets`／`rehearse`／`test:entry`／`test:ui`／`test:chat`／`test:workbench`／`test:runtime`／`test:gateway` 全過。First Load JS `/` 238 kB。
...
31. 進度徽章、任務列表、即時回覆與 Agent 進度列都改用同一套高階標籤。專案頁的活動表單收到「進階 · 活動與文案」。
32. 靈感頁來源匯入收到「進階 · 來源匯入」。學生畫面只留視覺語言卡與誠實公開來源聲明。Canva 與 Lumen 用不同圖示。外觀設定與設定分頁從 HermesConsole 拆出。連線頁不再把閘道環境變數名稱放在第一層。
33. Agent 頁不再標題「連線」；一般畫面只留 Hermes／記憶／工具／MCP 點，不顯示 12/20。客群第一眼標「模擬」，不是英文 SIMULATION。錯誤頁與瀏覽器標題不再寫 Creative Intelligence。
34. 接續修改與選定方向不再把 workflow id、Canva 範本欄位或工具名稱塞進輸入框。學生只看到自然語句；選定結果仍由後端 PATCH 保存。
35. 離線、待確認、停止與工作區回覆改成學生看得懂的中文：另開對話重試、遠端是否已停仍無法確認、工作區資料。一般畫面不再把事件摘要或「未宣稱遠端已停止」寫上。共用記憶不再露出 DATABASE_URL／MCP 用語。誠實狀態不變。
36. 視覺卡不再寫 UNKNOWN 或 Instagram 像素尺寸；缺欄改「未提供」。學習地圖與會話歷史收到設定「進階 · 學習紀錄與會話」。資料模型仍保留 unknownFields。
