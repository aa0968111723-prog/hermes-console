...
- [ ] 語音說完後出現「說完了，請按送出」，不自動送出（契約 FakeSpeechRecognition；非正式真機 Android Chrome）
- [ ] 口語送出後方向卡可點；選方向／出圖／改暖規格來自 POST，`GET /api/workflows` 失敗仍保留規格與「沒有出圖」
- [ ] 選定規格後口說「今天社博在哪」走社團索引；「幫我出圖」誠實未出圖；「顏色改暖一點」同一件 V2
...
- [ ] 任務頁預覽與對話規格同一 revision；改暖／出圖後不得停在 select POST 的 V1 `design`
