...
- 靈感：`workspace_search_inspiration` 回已保存參考與社團視覺模式；禁止宣稱 Instagram 全站搜尋。
...
- 一般 UI 任務進度是 理解／研究／創作／完成。靈感工具結果以 keep／avoid 卡片顯示；讀圖結果以海報預覽與模擬修改方向顯示，不是 tool JSON。工具名稱、計數、schema、traceId 只在 Developer／任務詳情「技術資訊」。
