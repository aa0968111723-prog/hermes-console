...
> 現行產品：AuthGate（未登入 → Login → Hermes）。下文是 PR #11 時期交付紀錄，當時選擇免登入＋受控閘道。

最新變更與限制以 [README](../README.md) 與 [PRODUCTION](PRODUCTION.md) 為準。以下保留前階段交付範圍；不以過往測試數或示意資料代表本輪外部實機驗證。
...
- 對話／專案各自保留未送出的文字與附件。非同步上傳回到原草稿，不會因切換而加入另一個對話。
- 草稿只暫存在目前分頁記憶體，重新整理將清除，輸入區明確提示；它不是 Hermes 記憶，也不是已保存的會話。主動登出清除全部草稿，登入到期則可重新登入接續。
- 長文輸入區自動增高、刪除後縮回；可視高度變小時限制輸入區高度。合成 390×400 viewport 測試不等同手機虛擬鍵盤實測。
- 編輯／重試分支保留原訊息附件；向上閱讀時不因新串流輸出強制捲動，提供回到最新訊息。
- 設定／導覽面板有無障礙名稱，分類支援方向鍵、Home／End、Escape 及焦點返回；表格可由鍵盤水平捲動。
- 瀏覽器拒絕 localStorage／sessionStorage 不再造成登入或外觀設定崩潰；登入失效不再被當成服務離線。
- 上傳增加逾時與取消清理；工具已取消／結果未知不再觸發龜龜搜尋動畫。Hermes 歷史畫面綁定查詢時的對話範圍。
- 新增真實瀏覽器檢查：延遲傳回**實際 Console 上傳結果**後切換對話、草稿分流、登入到期／登出、禁止儲存、面板鍵盤、輸入尺寸、分支附件、閱讀位置；不是外部平台的實機驗證。
- 另附 settings-desktop、settings-mobile-390 設定面板實際截圖。
...
本地結果：22 項後端／MCP／Canva 契約與角色狀態測試通過；上述兩套瀏覽器檢查通過；正式建置與型別檢查通過；套件稽核 0 項已知漏洞；秘密比對掃描通過。CI 結果以 PR 中的執行紀錄為準。
