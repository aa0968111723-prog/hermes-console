...
接續已合併 PR #11，並合併最新 main (`ef1df0f`，含 PR #12 及四份試算表匯入)；依使用者本輪明確選擇「免登入＋受控存取閘道」。不部署、不合併其他未審分支、不發佈社群。
...
- 修復新合併試算表 CSV 語法；匯入改為明確操作的受保護 POST，GET 不連網或寫入。控制 Google 轉址、大小、逾時；分來源處理失敗，保存實際結果；重試不覆蓋收藏。

### 驗證

- 53 項後端／官方 SDK／HTTP 契約與角色測試；試算表採隔離 fetch fixture，不是四份 Google 文件的實機驗證。
...
- 額外真實 Google CSV 讀取（隔離 SQLite、無 OAuth、不更改分享權限）：兩次執行累計保存 138 筆，第二次略過既有 12 筆；社課知識庫仍 network_error。原始資料未提交，這不是 Hermes／Canva 網宣端到端成功。
