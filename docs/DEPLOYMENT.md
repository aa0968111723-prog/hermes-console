...
- **日常金鑰也可在 Console「設定 → 連線」填寫**：Hermes 網址／金鑰、MCP 橋接權杖、核准 MCP JSON、場圖 Atlas 網址／權杖、訊核 MCP 網址／權杖、Lumen 創作台網址／權杖、淡江 MCP 網址／權杖。前端只收集後 POST 到 `/api/settings/credentials`，後端加密寫入 `CONSOLE_DATA_DIR`，執行期覆寫同名環境變數。GET 只回傳是否已設定與末四碼，不回傳完整秘密。
