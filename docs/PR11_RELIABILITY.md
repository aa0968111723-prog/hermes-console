...
接續已合併的 PR #11，並合併 main `ef1df0f`（含 PR #12 與試算表來源）。保留 Agent Registry、Session Key、明亮介面與龜龜；依使用者本輪選擇，採 **免登入＋受控存取閘道**。沒有部署、社群發佈或使用已公开舊憑證。
...
- **試算表匯入**：修正最新 main CSV 語法。靈感頁明確點擊才以受保護 POST 讀取四份既有來源，GET 不再自動匯入。HTTPS 轉址僅限 Google 文件內容主機、每份 2 MiB／20 秒上限；各來源並行，單一失敗不阻止其他來源。真實讀到新列才建立對應專案；專案與來源去重，既有手動修改保留。最近結果保存於 SQLite，重開可查看；匯入文字不代表視覺分析或公開使用授權。沒有修改 Google 文件分享權限。
...
- `npm test`：53 項，包括新增閘道拒絕、任意 MCP 目標／秘密配對拒絕、靈感跨專案及 ID 碰撞、真正 HTTP 創作任務、需求重試、用量去重、工具停止／範圍／預算與實際圖片解碼。新增 CSV 引號／換行／BOM、GET 無副作用、403 部分失敗、匯入重試保留修改、轉址與大小拒絕。Hermes／Canva 使用隔離協定伺服器，Sheets 使用 fetch fixture，**不是實機平台驗證**。
...
### Google 試算表實際讀取（非 fixture）

2026-09-06 13:50–13:51 Asia/Taipei，以隔離 SQLite 呼叫同一個後端匯入函式，使用真實 Google CSV 網路回應、無 OAuth、不修改分享權限。首次保存 12 筆、兩來源失敗；重試讀取 138 筆、新增 126、略過既有 12，社課知識庫仍回 network_error。總共三來源／138 筆保存成功；沒有將原始試算表內容或測試資料庫提交到 Git。這僅驗證真實來源 → 文字解析 → 專案收藏 → 重試去重；不代表內容已核對、圖片已分析、Canva 已製作或正式工作區已匯入。

真實 Chromium 檢查分別重跑通過；首次同時啟動 UI／聊天套件遇到 5 秒載入斷言逾時，獨立重跑通過。新匯入錯誤 UI 使用明確標示的回應 fixture；API 本體另由後端測試與上列真實來源執行驗證。
...
`POST /api/inspiration` 支援 `{ "action": "sync_sheets" }`；四份既有設定来源分別匯入自己的專案，不接受客戶端任意文件端點。GET 只讀取收藏與最後一次結果；並行重複提交共用一次執行。匯入目前由單一 Console 程序執行，不是跨副本佇列；程序中斷後需重試，已保存列會去重。這不代表 Gemini Spark 已完成實際授權連接。
