...
test("authenticated workspace entry contracts", async (t) => {
  ...
  await t.test("workspace APIs require a session", () => {
    assert.throws(
  ...
  await t.test("health and ready stay public; workspace needs auth", async () => {
    assert.equal((await workspace.GET(request("workspace", "GET", undefined, process.env.CONSOLE_ORIGIN, false))).status, 401);
    assert.equal((await health.GET(request("health", "GET", undefined, process.env.CONSOLE_ORIGIN, false))).status, 200);
    ...
    assert.equal((await credentials.GET(request("settings/credentials"))).status, 200);
  ...
  await t.test("loopback development still needs a session", () => {
  ...
  await t.test("confirmation tokens remain active", async () => {
    ...
    const created = await conversations.POST(
      request("conversations", "POST", { title: "登入後對話" }),
    );
