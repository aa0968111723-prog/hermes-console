...
const captured: { path: string; headers: Record<string, string | string[] | undefined> }[] =
  [];
...
test("no-login workspace, confirmation, discovery and creative intelligence", async (t) => {
  await t.test("anonymous APIs do not require login", async () => {
    ...
    assert.equal(
      (await authRoute.POST(request("auth", "POST", { username: "x", password: "y" })))
        .status,
      410,
    );
  ...
  await t.test("origin, size and client credentials still enforced", async () => {
    assert.equal(
      (await workspace.POST(request("workspace", "POST", { name: "x" }, false)))
        .status,
      403,
    );
    assert.equal(
      (
        await healthRoute.POST(
          request("health", "POST", { baseUrl: "http://169.254.169.254", apiKey: "x" }),
        )
      ).status,
      400,
    );
  ...
  await t.test("sessions/memory omitted stay unsupported; brain hidden", async () => {
  ...
  await t.test("Audience Twin splits evidence and scores stay 0-100", () => {
    ...
    const scores = audience.normalizeScores({ stopRate: 140, comprehension: -2 });
    assert.equal(scores.scores.stopRate, 100);
    assert.equal(scores.scores.comprehension, 0);
    assert.equal(scores.disclaimer, "AI 模擬評估，不代表真實市場調查。");
    assert.equal(audience.wantsReverseThinking("路人會不會滑掉"), true);
  });

  await t.test("IG and Pinterest URL ingest never claims full-site search", () => {
  ...
  await t.test("tool permissions and prompt injection wrapping", () => {
    ...
    assert.equal(permissions.permissionClass("canva_create_selected_draft"), "write");
  ...
  await t.test("usage aggregation does not invent cost; Canva unconfigured is honest", () => {
    usage.recordUsage({
      agentId: "general",
      model: "fixture-agent",
      projectId: "personal",
