...
const calls: Array<{ method?: string; id?: unknown; name?: string }> = [];
const lab = createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    calls.push({ method: body.method, id: body.id, name: body.params?.name });
    res.setHeader("Content-Type", "application/json");
    res.setHeader("mcp-session-id", "sess-clab");
...
test("ConsistencyLab auto-registers and is callable through clab_ workspace tools", async (t) => {
  ...
  await t.test("workspace toolsList exposes clab_get_project_tree", () => {
    ...
    assert.ok(names.includes("clab_list_tools"));
  ...
  await t.test("clab_ tools/call forwards JSON-RPC to ConsistencyLab", async () => {
    const result = await callTool(WORKSPACE_OWNER, "clab_get_project_tree", { projectId: "demo" });
    ...
    assert.ok(calls.some((c) => c.method === "initialize"));
    assert.ok(calls.some((c) => c.method === "notifications/initialized" && c.id === undefined));
    assert.ok(calls.some((c) => c.name === "get_project_tree"));
  });

  await t.test("clab_list_tools 走 tools/list", async () => {
    const listed = await invokeConsistencylab("list_tools", {});
    assert.equal(listed.count, 1);
    assert.equal(listed.workspacePrefix, "clab_");
  });
