...
export function routeToolsets(intent: string) {
  ...
  if (/逐格|中間張|時間軸|framelab|inbetween/i.test(intent)) selected.push("framelab");
  ...
  return {
    ...
    note: unique.includes("consistencylab") && unique.includes("framelab")
      ? "連戲／Golden 走 ConsistencyLab clab_*；逐格／中間張走 FrameLab framelab_*。GitHub 倉庫網址不是 MCP。"
      : unique.includes("consistencylab")
        ? "連戲／角色／Golden 走 ConsistencyLab MCP（clab_*）；未設定 endpoint 時工具代理會回未設定。"
        : unique.includes("framelab")
          ? "逐格／中間張走 FrameLab MCP（framelab_*）。GitHub 倉庫網址不是 MCP。"
