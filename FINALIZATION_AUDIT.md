...
| Android 鍵盤 | live（模擬 visualViewport） | 開啟才寫 `--app-height`；關閉移除，避免殼層永久縮短。`offsetTop` 納入判斷。鍵盤開啟時隱藏首頁龜龜／快捷鍵，輸入列捲到游標行，送出鍵仍在可視區。登入頁同樣隱藏龜龜／SSO，電子信箱、密碼與「進入工作區」留在可視區。不是實體 Android Chrome。 |
| Bottom dock | live | 對話／專案／Hermes／靈感／Agent；設定在頭像。任務在漢堡抽屜，不佔 360px 頂欄。漢堡不重複 Dock 分頁。頂欄另有開啟新對話。未連線時頂欄顯示「未設定」。手機頂欄不顯示會被裁成「He／Ag／靈／專案與」的標題；頁面 h1 與 Dock 負責定位。 |
| AuthGate | live（正式必填） | `/` → Login → Hermes。未登入不靜態載入 `HermesConsole`（`next/dynamic` + `ssr: false`）。Session 5xx／斷線 fail-closed：顯示「離線」與「再試一次」，不把錯誤 JSON 當成 workspace 模式、不載入工作區。First Load JS 約 107 kB；工作區 chunk 登入後才抓。Playwright 斷言 login 不請求該 chunk。`CONSOLE_AUTH_MODE=workspace` 僅契約／本機。 |
...
| 未連線送出 | live 契約 | credential 不是 valid 時不建立對話。提示「Hermes 還沒連上」並「前往連線」打開 Hermes 憑證欄。送出鍵未連線時不是綠色成功態，仍可點出誠實提示。 |
...
| 靈感結果 UI | live 契約 | 對話用 keep／avoid 卡片選方向；標已保存來源，不把 JSON 或全站搜尋當成成功。 |
...
| 對話殼層 | live | `TopBar`／`Conversation` 從 `HermesConsole` 拆出；方向挑選仍走同一 `pickDirection` → PATCH `/api/workflows`。Composer 增高時若已在底部會釘住；「回到最新訊息」在輸入列上方，不蓋住方向卡。 |
| 任務頁 | live | 與對話同一套 `DirectionPick`／`ArtifactDeck`；不再另做長文案方向表。 |
