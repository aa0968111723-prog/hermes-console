...
| AuthGate | live（正式必填） | `/` → Login → Hermes。未登入不靜態載入 `HermesConsole`（`next/dynamic` + `ssr: false`）。First Load JS 約 107 kB；工作區 chunk 登入後才抓。`CONSOLE_AUTH_MODE=workspace` 僅契約／本機。 |
