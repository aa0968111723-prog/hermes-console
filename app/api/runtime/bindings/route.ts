...
export const GET = route(async req => { const owner = authenticateOperator(req); return respond({ bindings: listRuntimeBindings(owner, new URL(req.url).searchParams.get("projectId") || undefined) }); });
export const POST = route(async req => { const owner = authenticateOperator(req, true); return respond({ binding: saveRuntimeBinding(owner, input.parse(await jsonBody(req, 20_000))) }, 201); });
